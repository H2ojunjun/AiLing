...
public class PlayerController : MonoSingleton<PlayerController>
{
    ...
    void Start()
    {
        ...
        _radius = _col.radius * 0.9f;
        overLapCapsuleOffset =  -(_col.height/2+0.5f);
        igonreLayer = ~(1 << LayerMask.NameToLayer("Player"));
    ...
    private void Move()
    {
        ...
        cc.Move(cc.transform.TransformDirection(movement.moveVec * Time.deltaTime));
    }
    private void CheckOnGround()
    {
        if (_groundCheckLock > 0)
            return;
        _bottom = transform.position + transform.up * _radius + transform.up * overLapCapsuleOffset;
        _top= transform.position + transform.up * _col.height/2 - transform.up * _radius;
