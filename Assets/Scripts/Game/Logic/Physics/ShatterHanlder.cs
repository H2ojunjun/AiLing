...
namespace AiLing
{
	public delegate void PostSplitDelegate(GameObject[] newGameObjects);
	public delegate void PreSplitDelegate(Plane[] planes);
	...
	public class ShatterHanlder : MonoBehaviour
    {
        ...
        private void Start()
        {
			_st = gameObject.GetComponent<ShatterTool>();
        }
		...
		{
			postSplitCallBack?.Invoke(newGameObjects);
            if (!_st.IsDestory)
            {
				foreach (var obj in newGameObjects)
				{
					obj.transform.parent = newTrans;
				}
			}
            else
            {
